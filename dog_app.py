...
human_count_from_dataset = np.sum([face_detector(i) for i in human_files_short])
human_count_from_dog_dataset = np.sum([face_detector(i) for i in dog_files_short])

# calculate and print percentage of faces in each sets
print('Human faces in human dataset detected: {}%'.format(human_count_from_dataset))
print('Human faces in dog dataset detected:   {}%'.format(human_count_from_dog_dataset))
...
param_data_directory = "dogImages"
...
# **Answer**:I loaded the training, test and validation datas, then I created DataLoaders for each of these sets of datas. After this, I resized all image to 224 pixel, center cropped, add randomly horizontal / vertical flip / rotations for some degrees to avoid overfitting of the model.
# 
...
# *__Answer*:__The first layer has input size of (224, 224, 3) and last layer should have the output size of 133 classes.
# 
...
# ```
# 
# __Answer:__ I think it is very efficient to use pre-trained networks and solve many problems in computer vision.
# 
...
# load the model that got the best validation accuracy (zuncomment the line below)
model_transfer.load_state_dict(torch.load('model_transfer.pt'))
